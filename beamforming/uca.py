...
#############################################
    ...
    def beamforming(self, chunks):
        ...
        for chunk in chunks:
            ...
            tf_sigs = np.fft.rfft(tf_sigs, len_of_sig)

                        # setting led && logger info
            pixel_ring.set_direction(direction[0])

##################################################
            logger.debug('@ {:.2f} @ {:.2f}, delays = {}'.format(direction[0], direction[1], np.array(delays)*self.fs))
...
###################################################
            ...
            # frq
            kappa = np.array([np.cos(np.deg2rad(direction[0]))*np.sin(np.deg2rad(90-direction[1])), np.sin(np.deg2rad(direction[0]))*np.sin(np.deg2rad(90-direction[1])), np,cos(np.deg2rad(90-direction[1]))] )
            df = self.fs/len_of_sig
            frq = arange(0, (self.fs/2) + df, df)
            k = 2*np.pi*frq/343.2
            # create Rxx
            Rxx = np.zeros((6,6,len(frq)))
            for i in range(1, 7)# 6 microphones
                for j in range(1,7)
                    s_Rxx = np.abs(sum(GD_matrix[:,i-1]-GD_matrix[:,j-1], 2))/np.pi*k
                    Rxx[i-1][j-1][:]=np.sin(s_Rxx)/(s_Rxx)

            # scan frq
            for i in range(1,len(frq)+1)
                A = math.exp(1j*np.dot(k[i-1],kappa*GD_matrix)) # 1x6
                W = np.dot((np.linalg.inv( Rxx[:][:][i-1] + 0.00001)).T.conj(), np.linalg.inv( Rxx[:][:][i-1] + 0.00001))
                source_half[i-1] = np.dot((W[:,i-1]).T.conj() , tf_sigs[:,i-1] )

            enhanced_speech.append(np.fft.irfft(source_half, len_of_sig))
